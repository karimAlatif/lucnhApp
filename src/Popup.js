...
const numBlock = (props) => {
  ...
  return (
    <div ref={forwardedRef} >
      {
        inViewport && <div style={{marginLeft:"5%"}} >
          {
            data.map((item,index)=>{      
      ...
      <div className="grid-container">
        {
          data.map( (item,index)=>{
            return <div key={index} className="grid-item col3">
              <img data-aos="zoom-in" src={item.imgPath} alt="Trulli" width="75" height="75"></img>
              <strong>{item.type}</strong>
              <br/>
              <br/>
              <p data-aos="fade-down" style={{fontSize:"20px", fontWeight:"900"}}>{item.name}</p>
              <p data-aos="fade-down" style={{fontSize:"15px", width:"80%", marginLeft:"10%"}} >{item.info}</p>
            </div>
          })
        }
      </div>
  ...
  return (
    ...
    <div id="mm" className="menu" onWheel = {(e) => {
      ...
      <div id = {sectionsTags[0].id} className="section">
        ...
        <p  data-aos="fade-down" style={{width:"80%", marginLeft:"10%"}} >
          {content.page1.paragraph}
       </p>
      </div>
      <div id = {sectionsTags[1].id} style={{paddingTop:"15vh"}} className="section">
      ...
      <div id = {sectionsTags[3].id} className="section" style={{paddingTop:"5vh"}}>
        <h1 data-aos="zoom-out" style={{marginBottom:"10px", paddingTop:"15vh"}} >{sectionsTags[3].name}</h1>
        {
          renderIcons(content.page4.data)
        }
      </div>
      <div id = {sectionsTags[4].id} className="section" style={{paddingTop:"11vh"}}>
        <h1 data-aos="zoom-out" style={{marginBottom:"10px"}} >{sectionsTags[4].name}</h1>
        {
      ...
      <div id = {sectionsTags[5].id} className="section">
        ...
        <img data-aos="zoom-in" style={{width:"70%", height:"50%"}} src= {content.page6.imgUrl} alt="Trulli"></img>
      </div>
      ...
      <div id = {sectionsTags[8].id} style={{paddingTop:"5vh"}} className="section">
        <h1 data-aos="zoom-out" style={{marginBottom:"10px"}} >Request a Demo</h1>
        <Form style={{marginTop:"10vh"}}>
          ...
          <Button href="https://share.hsforms.com/1JhBzAwM9Sty6NXYAWq3SoA2ctn2" style={{marginTop:"15px", width:"25%" }} variant="primary" type="submit"> 
            Submit
          </Button>
        </Form>
      </div>
